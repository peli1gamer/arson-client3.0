...
public final class FabricFeatureContextAdapter implements FeatureContextAdapter {
    ...
    public FabricFeatureContextAdapter(Minecraft client) {
        this.client = client;
    }
    ...
    public void initialize(FeatureContext context) {
        context.initialize();
        context.tick(snapshot());
    }

    @Override
    public void tick(FeatureContext context) {
        context.tick(snapshot());
    }

    @Override
    public void render(FeatureContext context, long frame, float partialTick) {
        context.render(frame, partialTick, RenderState.Phase.AFTER_ENTITIES);
    }

    @Override
    public void disconnect(FeatureContext context) {
        context.disconnect();
    }
