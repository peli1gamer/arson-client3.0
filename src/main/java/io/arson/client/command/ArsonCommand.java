        ...
        return ClientCommandManager.literal("hud")
            ...
            .then(ClientCommandManager.literal("element-format")
                .then(ClientCommandManager.argument("element", StringArgumentType.word())
                    ...
                    .executes(ctx -> {
                        String element = StringArgumentType.getString(ctx, "element");
                        var current = io.arson.client.api.ArsonApi.hudElementRowFormat(element);
                        if (current.isEmpty()) { error(ctx, "Unknown HUD information element"); return 0; }
                        HudModule hud = (HudModule) ArsonClient.getInstance().modules().get("hud");
                        feedback(ctx, element + " row format: " + current.get().name().toLowerCase(java.util.Locale.ROOT)
                            + " (effective " + hud.elementRowFormat(element).name().toLowerCase(java.util.Locale.ROOT) + ")");
                        return 1;
                    })
                    .then(ClientCommandManager.argument("format", StringArgumentType.word())
                        .suggests((ctx, builder) -> { for (HudModule.ElementRowFormat format : HudModule.ElementRowFormat.values()) builder.suggest(format.name().toLowerCase(java.util.Locale.ROOT)); return builder.buildFuture(); })
                        .executes(ctx -> {
                            String element = StringArgumentType.getString(ctx, "element");
                            String raw = StringArgumentType.getString(ctx, "format");
                            HudModule.ElementRowFormat format;
                            try { format = HudModule.ElementRowFormat.valueOf(raw.toUpperCase(java.util.Locale.ROOT)); }
                            catch (IllegalArgumentException exception) { error(ctx, "Unknown HUD row format"); return 0; }
                            if (!io.arson.client.api.ArsonApi.setHudElementRowFormat(element, format)) { error(ctx, "Unknown HUD information element"); return 0; }
                            feedback(ctx, element + " row format set to " + format.name().toLowerCase(java.util.Locale.ROOT));
                            return 1;
                        }))))
            .then(ClientCommandManager.literal("row-format")
                .then(ClientCommandManager.argument("format", StringArgumentType.word())
                    .suggests((ctx, builder) -> { for (HudModule.RowFormat format : HudModule.RowFormat.values()) builder.suggest(format.name().toLowerCase(java.util.Locale.ROOT)); return builder.buildFuture(); })
                    .executes(ctx -> {
                        try {
                            HudModule.RowFormat format = HudModule.RowFormat.valueOf(StringArgumentType.getString(ctx, "format").toUpperCase(java.util.Locale.ROOT));
                            if (!io.arson.client.api.ArsonApi.setHudRowFormat(format)) { error(ctx, "HUD unavailable"); return 0; }
                            feedback(ctx, "HUD row format: " + format.name().toLowerCase(java.util.Locale.ROOT));
                            return 1;
                        } catch (IllegalArgumentException exception) { error(ctx, "Unknown row format"); return 0; }
                    })))
            .then(ClientCommandManager.literal("preset")
                .then(ClientCommandManager.argument("name", StringArgumentType.word())
                    .suggests((ctx, builder) -> { builder.suggest("minimal"); builder.suggest("compact"); builder.suggest("full"); return builder.buildFuture(); })
                    .executes(ctx -> {
