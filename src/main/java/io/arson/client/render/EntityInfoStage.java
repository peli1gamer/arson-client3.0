...
public final class EntityInfoStage implements WorldRenderBridge.WorldRenderStage {
    ...
    private static void fillQuad(PoseStack matrices,MultiBufferSource consumers,float x1,float y1,float x2,float y2,int argb){var pose=matrices.last();var buffer=consumers.getBuffer(RenderTypes.guiText());float a=((argb>>>24)&255)/255f,r=((argb>>>16)&255)/255f,g=((argb>>>8)&255)/255f,b=(argb&255)/255f;buffer.addVertex(pose,x1,y1,0).setColor(r,g,b,a);buffer.addVertex(pose,x1,y2,0).setColor(r,g,b,a);buffer.addVertex(pose,x2,y2,0).setColor(r,g,b,a);buffer.addVertex(pose,x2,y1,0).setColor(r,g,b,a);}
}
