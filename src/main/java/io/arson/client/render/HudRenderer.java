...
public final class HudRenderer {
    ...
    public static void render(GuiGraphics graphics, DeltaTracker tickCounter) {
        ...
        if (hud.showPlayerInfo() && playerInfo != null && playerInfo.enabled()) {
            ...
            if (playerVitals != null && playerVitals.enabled()) {
                ...
                rows.add(playerVitals.formattedExperience());
                rows.add(playerVitals.formattedAir());
    ...
    private static void drawElement(GuiGraphics graphics, Minecraft client, HudModule hud, HudLayoutModule layout, String element, int fallbackX, int fallbackY, String[] rows) {
        ...
        if (layout != null) {
            ...
            double[] resolved = layout.resolve(element, logicalViewportW, logicalViewportH, maxWidth * elementScale, rows.length * line * elementScale);
            x = resolved[0]; y = resolved[1];
        }
        graphics.pose().pushMatrix(); graphics.pose().translate((float)x, (float)y); graphics.pose().scale((float) elementScale, (float) elementScale);
        int padding = hud.padding(); String alignment = hud.elementAlignment(element); int left = alignedX(0, maxWidth, alignment);
