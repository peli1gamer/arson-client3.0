...
public final class HudRenderer {
    ...
    public static void render(GuiGraphics graphics, DeltaTracker tickCounter) {
        ...
        if (hud.showPlayerInfo() && playerInfo != null && playerInfo.enabled()) { java.util.ArrayList<String> rows = new java.util.ArrayList<>(); if (playerInfo.showHealth()) rows.add(String.format(java.util.Locale.ROOT, "Health %.1f/%.1f", client.player.getHealth(), client.player.getMaxHealth())); if (playerInfo.showHunger()) rows.add("Food " + client.player.getFoodData().getFoodLevel()); if (playerInfo.showArmor()) rows.add(armor(client)); if (playerInfo.showHeldItem()) { ItemStack stack = client.player.getMainHandItem(); rows.add(stack.isEmpty() ? "Held Hand" : "Held " + stack.getHoverName().getString()); } drawElement(graphics, client, hud, "player-info", hud.playerInfoX(), hud.playerInfoY(), rows.toArray(String[]::new)); }
        if (hud.showWorldInfo() && worldInfo != null && worldInfo.enabled()) { java.util.ArrayList<String> rows = new java.util.ArrayList<>(); if (worldInfo.showTime()) { long dayTime = Math.floorMod(client.level.getDayTime(), 24000L); long hours = (dayTime / 1000L + 6L) % 24L; long minutes = Math.round((dayTime % 1000L) * 60.0 / 1000.0); if (minutes == 60) { minutes = 0; hours = (hours + 1) % 24; } rows.add(String.format(java.util.Locale.ROOT, "Time %02d:%02d", hours, minutes)); } if (worldInfo.showDimension()) rows.add("Dimension " + client.level.dimension().identifier()); if (worldInfo.showWeather()) rows.add("Weather " + (client.level.isThundering() ? "Thunder" : client.level.isRaining() ? "Rain" : "Clear")); drawElement(graphics, client, hud, "world-info", hud.worldInfoX(), hud.worldInfoY(), rows.toArray(String[]::new)); }
        graphics.pose().popMatrix();
    }
    private static void renderNotifications(GuiGraphics graphics, Minecraft client) {
        var list = NotificationCenter.active(System.currentTimeMillis());
        int screenW = client.getWindow().getGuiScaledWidth(), screenH = client.getWindow().getGuiScaledHeight();
        int index = 0;
        for (NotificationCenter.Notification n : list) {
            int width = Math.max(180, Math.max(client.font.width(n.title()), client.font.width(n.message())) + 24), height = 40;
            float p = n.progress(System.currentTimeMillis());
            float eased = p * p * (3f - 2f * p);
            int slide = Math.round((1f - eased) * 18f);
            boolean left = NotificationCenter.position() == NotificationCenter.Position.TOP_LEFT || NotificationCenter.position() == NotificationCenter.Position.BOTTOM_LEFT;
            boolean bottom = NotificationCenter.position() == NotificationCenter.Position.BOTTOM_LEFT || NotificationCenter.position() == NotificationCenter.Position.BOTTOM_RIGHT;
            int x = left ? 10 : screenW - width - 10;
            int y = bottom ? screenH - 10 - height - index * 45 : 10 + index * 45;
            x += left ? -slide : slide;
            int outline = switch (n.priority()) { case HIGH -> 0xFFFF5555; case LOW -> 0xFF777777; case NORMAL -> 0xFF5555AA; };
            graphics.fill(x, y, x + width, y + height, 0xE0181820);
            graphics.renderOutline(x, y, width, height, outline);
            graphics.drawString(client.font, n.title(), x + 8, y + 6, 0xFFFFFFFF);
            graphics.drawString(client.font, n.message(), x + 8, y + 20, 0xFFD0D0D0);
            graphics.fill(x + 1, y + height - 3, x + 1 + Math.round((width - 2) * eased), y + height - 1, outline);
            index++;
        }
    }
    private static void drawElement(GuiGraphics graphics, Minecraft client, HudModule hud, String element, int x, int y, String[] rows) { if (rows.length == 0) return; double elementScale = hud.elementScale(element); graphics.pose().pushMatrix(); graphics.pose().translate(x, y); graphics.pose().scale((float) elementScale, (float) elementScale); int maxWidth = 0; for (String row : rows) maxWidth = Math.max(maxWidth, client.font.width(row)); int padding = hud.padding(), line = hud.lineSpacing(); String alignment = hud.elementAlignment(element); int left = alignedX(0, maxWidth, alignment); if (hud.showBackground() && hud.elementBackground(element)) graphics.fill(left - padding, -padding, left + maxWidth + padding, rows.length * line + padding - 1, hud.elementBackgroundColor()); for (int i = 0; i < rows.length; i++) { int rowWidth = client.font.width(rows[i]); int rowX = alignedX(0, rowWidth, alignment); graphics.drawString(client.font, rows[i], rowX, i * line, hud.elementColor(element), hud.showShadow()); } graphics.pose().popMatrix(); }
